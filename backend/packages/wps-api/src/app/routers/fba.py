...
async def get_all_zone_data_for_source_ids(
    ...
    # One query for every zone, rather than one query per zone.
    stats_by_source_id = await get_precomputed_stats_for_shapes(
        session,
    ...
    zones_without_stats = [
        ...
        if not stats_by_source_id.get(int(zone_source_id))
    ]
    if zones_without_stats:
        ...
        prev_stats_by_source_id = await get_precomputed_stats_for_shapes(
            session,
            run_type=RunTypeEnum(run_type.value),
            for_date=for_date,
            run_datetime=run_datetime,
            source_identifiers=zones_without_stats,
            fuel_type_raster_id=prev_fuel_type_raster.id,
        )
        for zone_source_id in zones_without_stats:
            prev_stats = prev_stats_by_source_id.get(int(zone_source_id))
            if prev_stats:
                stats_by_source_id[int(zone_source_id)] = prev_stats

    all_zone_data: dict[int, FireZoneHFIStats] = {}
    for zone_source_id in zone_source_ids:
        hfi_fuel_type_ids_for_zone = stats_by_source_id.get(int(zone_source_id), [])
